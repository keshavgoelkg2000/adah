...
import com.auctionhub.dto.ProductAddRequest;
import com.auctionhub.dto.ProductDetailUpdateRequest;
import com.auctionhub.dto.ProductResponseDto;
import com.auctionhub.entity.Category;
import com.auctionhub.entity.Product;
import com.auctionhub.entity.User;
import com.auctionhub.exception.ProductSaveFailedException;
import com.auctionhub.service.CategoryService;
import com.auctionhub.service.ProductService;
import com.auctionhub.service.StorageService;
...
public class ProductResource {
	...
	public ResponseEntity<CommonApiResponse> updateProductDetail(ProductDetailUpdateRequest request) {
		...
		if (request.getCategoryId() != 0 && product.getCategory().getId() != request.getCategoryId()) {
			Category category = this.categoryService.getCategoryById(request.getCategoryId());
			product.setCategory(category);
		}

		if (Long.parseLong(request.getEndDate()) < Long.parseLong(product.getEndDate())) {
			response.setResponseMessage("You can only Extend the Expiry Date");
			response.setSuccess(false);
		...
		// product.setPrice(request.getPrice()); // price we will not udpate, because
		// buyer has bidden on this considering the actual price
	...
	// we can only delete the product, whose status is Available --> means which
	// available on Website for Bidding
	// but if it is SOLD, UNSOLD or Already Deactivated then we'll not delete that
	public ResponseEntity<CommonApiResponse> deleteProduct(int productId, int sellerId) {
		...
		if (!product.getStatus().equals(ProductStatus.AVAILABLE.value())) {
			response.setResponseMessage("Product Not Avaibale for Sell, Can't Delete product");
			response.setSuccess(false);
	...
	public ResponseEntity<ProductResponseDto> fetchAllProducts(String status) {
		...
		if (StringUtils.isEmpty(status)) {
			statuses = Arrays.asList(ProductStatus.AVAILABLE.value(), ProductStatus.SOLD.value(),
					ProductStatus.UNSOLD.value(), ProductStatus.DEACTIVATED.value());
		} else {
			statuses = Arrays.asList(status);
		}

		List<Product> products = this.productService.getAllProductByStatusIn(statuses);
	...
	public ResponseEntity<ProductResponseDto> fetchAllSellerProducts(int sellerId, String status) {
		...
		List<Product> products = this.productService.getAllProductBySellerAndStatusIn(seller, statuses);
